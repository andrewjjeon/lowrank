...
class CustomDataset(Dataset):
    def __init__(self, X, Y):
        self.X = torch.tensor(X, device='cpu').float()
        self.Y = torch.tensor(Y, device='cpu').float()
...
def estimate_gd_nuc(X,Y,n_iters=None,reg=0.00001,lr=None,step_multiplier=None):
    ...
    _,d = X.shape
    Y = torch.tensor(Y, device='cpu').float()
    data_loader = DataLoader(dataset=data, batch_size=batch_size, shuffle=True, drop_last=len(X)>batch_size, generator=torch.Generator(device='cpu'))

    #u_hat2 = estimate_ls(X,Y)
    u_hat2 = 0.1*np.random.randn(d,d)
    u_hat = torch.tensor(u_hat2, device='cpu').float()
    u_hat.requires_grad_()
    if lr is None:
...
def estimate_gd_nuc_project_AB(X,Y,reg=0.00001,lr=0.0001,n_iters=1000,transfer0=None):
    ...
    if transfer0 is None:
        ...
        A_hat = transfer0[0]
        B_hat = transfer0[1]
    A_hat = torch.tensor(A_hat, device='cpu').float()
    B_hat = torch.tensor(A_hat, device='cpu').float()
    ...
    plt.title("nuc estimation loss")
    plt.show()
    return A_hat.detach().numpy(), B_hat.detach().numpy()
...
def estimate_gd_nuc_project_AB_offset(X,Y,reg=0.00001,lr=0.0001,n_iters=1000,transfer0=None,plt_save=None):
    ...
    if transfer0 is None:
        ...
        v_hat = transfer0[2]
    A_hat = torch.tensor(A_hat, device='cpu').float()
    B_hat = torch.tensor(B_hat, device='cpu').float()
    v_hat = torch.tensor(v_hat, device='cpu').float()
    U,S,V = torch.linalg.svd(B_hat)
    S = l1_proj(S, max_on=reg)
    B_hat.data = U @ torch.diag(S) @ V
    A_hat.requires_grad_()
    ...
    if n_iters > 2:
        plt.plot(loss_vals)
        plt.title("nuc estimation loss")
        plt.savefig('./results/loss.png') #show()
        if plt_save is not None:
            plt.savefig(plt_save + ".png")
            plt.close()
    return A_hat.detach().numpy(), B_hat.detach().numpy(), v_hat.detach().numpy()



def estimate_gd_nuc_project_AB_offset_sigmoid(X,Y,reg=0.00001,lr=0.0001,n_iters=1000,transfer0=None,plt_save=None):
    Y = torch.tensor(Y, device='cpu').float()
    n_pts,d = Y.shape
    X = torch.tensor(X, device='cpu').float()
    X_B = X[:,0:d]
...
def lr_diag_decomp(A_gt, reg):
    ...
    A_opt = torch.tensor(A_opt, device='cpu').float()
...
def estimate_gd_nuc_project_diag(X,Y,reg=0.00001,lr=None,n_iters=1000,transfer0=None,plt_save=None):
    ...
    A = torch.tensor(A, device='cpu').float()
    D = torch.tensor(D, device='cpu').float()
    ...
    if plt_save is not None:
        plt.savefig(plt_save + ".png")
        plt.close()
    return (A + torch.diag(D)).detach().numpy(), torch.diag(D).detach().numpy(), A.detach().numpy()
...
def estimate_gd_lowrank(X,Y,n_iters=None,rank=5,lr=None,step_multiplier=None,transfer0=None):
    ...
    if transfer0 is None:
        U = torch.randn(d,rank, device='cpu').float()
        V = torch.randn(d,rank, device='cpu').float()
        U.requires_grad_()
        V.requires_grad_()
    else:
        U0,S0,V0 = la.svd(transfer0)
        U = torch.tensor(U0[:,0:rank] @ np.diag(np.sqrt(S0[0:rank])), device='cpu').float()
        V = torch.tensor(V0[:,0:rank] @ np.diag(np.sqrt(S0[0:rank])), device='cpu').float()
        U.requires_grad_()
        V.requires_grad_()
    if lr is None:
...
#         epoch_loss += loss_val
        ...
        if min_loss is None:
            min_loss = epoch_loss
            best_connect = U.clone().detach().numpy() @ V.clone().detach().numpy().T
...
def estimate_gd_lowrank_alternate(X,Y,n_iters=None,rank=5,lr=None,step_multiplier=None,transfer0=None):
    ...
    if n_iters is None:
        ...
        plt.show()
    connect = U @ V.T
    return connect.clone().detach().numpy()
...
def estimate_gd_lowrank_alternate2(X,Y,n_iters=None,rank=5,lr=None,step_multiplier=None,transfer0=None):
    ...
    if transfer0 is None:
        ...
        U = torch.tensor(U0[:,0:rank] @ np.diag(np.sqrt(S0[0:rank])), device='cpu').float()
        V = torch.tensor(V0[:,0:rank] @ np.diag(np.sqrt(S0[0:rank])), device='cpu').float()
        # U = torch.tensor(U0[:,0:rank], device='cpu').float()
        # V = torch.tensor(V0[:,0:rank] @ np.diag(S0[0:rank]), device='cpu').float()
        V.requires_grad_()
    if lr is None:
        #optimizer_U = optim.Adam([U], lr=0.025)
        optimizer_V = optim.Adam([V], lr=0.025)
...
def estimate_gd_lowrank_alternate2_diag(X,Y,n_iters=500,rank=5,lr=None,step_multiplier=None,transfer0=None):
    ...
    if transfer0 is None:
        U = torch.randn(d,rank, device='cpu').float()
        V = torch.randn(d,rank, device='cpu').float()
        D = torch.randn(d, device='cpu').float()
        V.requires_grad_()
        D.requires_grad_()
    else:
        ...
        U = torch.tensor(U0[:,0:rank] @ np.diag(np.sqrt(S0[0:rank])), device='cpu').float()
        V = torch.tensor(V0[:,0:rank] @ np.diag(np.sqrt(S0[0:rank])), device='cpu').float()
        D = torch.tensor(np.diag(transfer0), device='cpu').float()
        V.requires_grad_()
        D.requires_grad_()
    if lr is None:
...
def estimate_gd_lowrank_AB(X,Y,rank=10,lr=0.0001,n_iters=1000,transfer0=None):
    ...
    if transfer0 is None:
        U = torch.randn(d,rank, device='cpu').float()
        V = torch.randn(d,rank, device='cpu').float()
        D = torch.randn(d,d, device='cpu').float()
        V.requires_grad_()
        D.requires_grad_()
    else:
        ...
        U = torch.tensor(U0[:,0:rank] @ np.diag(np.sqrt(S0[0:rank])), device='cpu').float()
        V = torch.tensor(V0[:,0:rank] @ np.diag(np.sqrt(S0[0:rank])), device='cpu').float()
        D = torch.tensor(transfer0[0], device='cpu').float()
        V.requires_grad_()
        U.requires_grad_()
        D.requires_grad_()
    ...
    plt.show()
    B = U @ V.T
    return D.clone().detach().numpy(), B.clone().detach().numpy()
...
def estimate_gd_lowrank_AB_offset(X,Y,rank=10,lr=0.0001,n_iters=1000,transfer0=None):
    ...
    if transfer0 is None:
        ...
        V = torch.randn(d,rank, device='cpu').float()
        D = torch.randn(d,d, device='cpu').float()
        offset = torch.randn(d, device='cpu').float()
        V.requires_grad_()
        U.requires_grad_()
        D.requires_grad_()
        ...
        U = torch.tensor(U0[:,0:rank] @ np.diag(np.sqrt(S0[0:rank])), device='cpu').float()
        #V = torch.tensor(V0[:,0:rank] @ np.diag(np.sqrt(S0[0:rank])), device='cpu').float()
        V = torch.tensor(np.diag(np.sqrt(S0[0:rank])) @ V0[0:rank,:], device='cpu').float()
        D = torch.tensor(transfer0[0], device='cpu').float()
        offset = torch.tensor(transfer0[2], device='cpu').float()
        V.requires_grad_()
        U.requires_grad_()
        D.requires_grad_()
        offset.requires_grad_()
    ...
    plt.show()
    B = U @ V
    return D.clone().detach().numpy(), B.clone().detach().numpy(), offset.clone().detach().numpy()
...
def estimate_gd_lowrank_project_diag(X,Y,n_iters=500,rank=5,lr=0.025,step_multiplier=None,transfer0=None):
    ...
    if transfer0 is None:
        A = torch.randn(d,d, device='cpu').float()
        D = torch.randn(d, device='cpu').float()
        A.requires_grad_()
        D.requires_grad_()
    else:
        U0,S0,V0 = la.svd(transfer0)
        S0[rank:] = 0
        A = torch.tensor(U0 @ np.diag(S0) @ V0, device='cpu').float()
