...
def design_inputs_fisher(transfer_est, num_batches=1000, verbose=True, n_iters=1000, sphere_normalization=False, num_random=1000):
    ...
    for n in range(n_iters):
        ...
        if np.mod(n,5) == 0 and verbose:
            nnz_column = torch.sum(u_batch > 0.01, axis=0)
            print(n,loss_val,loss_exp,torch.mean(nnz_column.float()).cpu().numpy(),la.norm(u_batch.clone().detach().cpu().flatten().numpy(), 1)/num_batches)
            if torch.norm(u_batch.clone().detach().cpu(), 1)/num_batches < 14.5 and n > 50:
                break
        loss_vals.append(loss_val)
...
def design_inputs_constrained(transfer_est, 
    ...
    for n in range(n_iters):
        ...
        if np.mod(n,10) == 0 and verbose:
            nnz_column = torch.sum(u_batch > 0.01, axis=0)
            print(n,loss_val,torch.mean(nnz_column.float()).cpu().numpy(),la.norm(u_batch.clone().detach().cpu().flatten().numpy(), 1)/num_batches)
        loss_vals.append(loss_val)
